...
function App() {
  ...
  return <main className={mood ? 'phone-shell mood-active' : 'phone-shell'} style={moodStyle}>
    ...
    <nav className="nav-footer" aria-label="주요 메뉴">{tabs.map(([name, icon]) => <button key={name} className={tab === name ? 'tab active' : 'tab'} onClick={() => { setActivePlaylist(null); setDiaryWriting(false); setTab(name) }}><img src={icon} alt="" /><span>{name}</span></button>)}</nav>
