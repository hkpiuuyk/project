...
function TrackRow({ track, onPlay, light, onRowClick, onDelete }: { track: Track | readonly [string, string]; onPlay: () => void; light: boolean; onRowClick?: () => void; onDelete?: () => void }) {
  const displayTrack = (Array.isArray(track) ? { id: track[0], title: track[0], artist: track[1] } : track) as Track
  return <div className={onRowClick ? 'track-row clickable-row' : 'track-row'} onClick={onRowClick}>
    <div 
      className="artwork" 
      style={displayTrack.coverUrl ? { backgroundImage: `url(${displayTrack.coverUrl})`, backgroundSize: 'cover' } : undefined} 
      aria-hidden="true" 
    />
    ...
    {onDelete && <button className="playlist-delete" onClick={event => { event.stopPropagation(); onDelete() }} aria-label={`${displayTrack.title} 삭제`}>삭제</button>}
    <button className="icon-button play-button" onClick={event => { event.stopPropagation(); onPlay() }} aria-label={`${displayTrack.title} 재생`}><img src={light ? playLightIcon : playIcon} alt="" /></button>
  </div>
}
...
function App() {
  ...
  const selectTrack = (nextTrack: Track | readonly [string, string]) => {
    const selected = (Array.isArray(nextTrack) ? { id: nextTrack[0], title: nextTrack[0], artist: nextTrack[1] } : nextTrack) as Track
    setTrack(selected)
    setPlaying(true)
