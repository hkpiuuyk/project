...
function App() {
  ...
  const [track, setTrack] = useState<Track>(allSearchTracks[0] || initialTracks[0])
...
const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return <main className={mood ? 'phone-shell mood-active' : 'phone-shell'} style={moodStyle}>
    ...
    <section className={activePlaylist ? 'content playlist-detail-content' : tab === '탐색' || tab === '라이브러리' || tab === '일기' ? 'content search-content' : 'content'} aria-label={`${tab} 화면`}>
      {activePlaylist ? <><section className="detail-track-list">{detailTracks.map(item => <div className={draggedSong === item.title ? 'detail-track-row dragging' : 'detail-track-row'} data-song-name={item.title} key={item.id}><span className="artwork" style={item.coverUrl ? { backgroundImage: `url(${item.coverUrl})`, backgroundSize: 'cover' } : undefined} /><span className="track-copy"><strong>{item.title}</strong><span>{item.artist}</span></span><div className="song-actions"><button className="order-handle" onPointerDown={event => { event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); setDraggedSong(item.title) }} onPointerMove={event => { if (!draggedSong) return; const target = document.elementFromPoint(event.clientX, event.clientY)?.closest<HTMLElement>('[data-song-name]')?.dataset.songName; if (target && target !== draggedSong) movePlaylistSong(draggedSong, target) }} onPointerUp={() => setDraggedSong(null)} onPointerCancel={() => setDraggedSong(null)} aria-label={`${item.title} 순서 변경`}><img src={orderIcon} alt="" /></button><button className="song-delete-button" onClick={() => deletePlaylistSong(item.title)} aria-label={`${item.title} 삭제`}>삭제</button></div></div>)}</section><button className="add-song-button" onClick={() => { setTargetPlaylist(activePlaylist); setSelectedSongNames([]); setSongAddSheetOpen(true) }}>+ 곡 추가</button></> : diaryWriting ? <section className="diary-form"><div className="diary-date">오늘 · {new Date().toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/ /g, '')}</div><div className="diary-form-section"><h2>오늘의 기분</h2><div className="diary-mood-list">{moods.map(item => <button key={item.name} className={diaryMood === item.name ? 'diary-mood selected' : 'diary-mood'} onClick={() => setDiaryMood(item.name)}>{item.name}</button>)}</div></div><div className="diary-form-section"><h2>오늘의 이야기</h2><label className="diary-textarea"><textarea value={diaryText} onChange={event => setDiaryText(event.target.value.slice(0, 300))} placeholder="오늘 들은 음악과 마음을 기록해 보세요." aria-label="일기 내용" /><span>{diaryText.length}/300</span></label></div><div className="diary-form-section"><h2>함께 들은 곡</h2><div className="diary-selected-track"><span className="artwork" style={track.coverUrl ? { backgroundImage: `url(${track.coverUrl})`, backgroundSize: 'cover' } : undefined} /><span className="track-copy"><strong>{track.title}</strong><span>{track.artist}</span></span></div></div><button className="create-button diary-save-button" onClick={saveDiary}>저장하기</button></section> : tab === '홈' ? <>
        
        {/* 원클릭 MP3 자동 메타데이터 업로드 카드 */}
        <section className="music-section" style={{ background: '#f5f5f5', padding: '16px', borderRadius: '16px' }}>
          <h2>음악 파일 업로드</h2>
          <p style={{ margin: '4px 0 12px', fontSize: '12px', color: '#8a7a6d' }}>MP3 파일을 올려 보세요.</p>
          <label style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: '40px', padding: '0 16px', background: '#1c2024', color: '#fff', borderRadius: '10px', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
            음악 파일 선택 (.mp3)
        ...
        <section className="playlist-section"><div className="playlist-heading"><h2>내 플레이리스트</h2><button onClick={() => setCreateSheetOpen(true)}>+ 만들기</button></div>{playlists.map(item => <TrackRow key={item[0]} track={{ id: item[0], title: item[0], artist: item[1] }} light={false} onRowClick={() => setActivePlaylist(item[0])} onDelete={() => setPlaylistPendingDelete(item[0])} onPlay={() => selectTrack(allSearchTracks[0])} />)}</section>
      </> : tab === '일기' ? diaryEntries.length > 0 ? <section className="diary-list">{diaryEntries.map(entry => <article className="diary-card" key={`${entry.date}-${entry.text}`}><div className="diary-card-top"><time>{entry.date}</time><span>{entry.mood}</span></div><p>{entry.text}</p><div className="diary-track"><span className="artwork" style={entry.track.coverUrl ? { backgroundImage: `url(${entry.track.coverUrl})`, backgroundSize: 'cover' } : undefined} /><span className="track-copy"><strong>{entry.track.title}</strong><span>{entry.track.artist}</span></span><button className="icon-button play-button" onClick={() => selectTrack(entry.track)} aria-label={`${entry.track.title} 재생`}><img src={diaryPlayIcon} alt="" /></button></div></article>)}</section> : <section className="diary-empty"><span className="diary-empty-mark" aria-hidden="true" /><h2>아직 쓴 일기가 없어요</h2><p>오늘 들은 음악과 마음을 기록해 보세요.</p><button className="create-button" onClick={() => setDiaryWriting(true)}>첫 일기 쓰기</button></section> : tab === '탐색' ? <>
