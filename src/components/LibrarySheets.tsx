...
function CreatePlaylistSheet({ setCreateSheetOpen, playlistName, setPlaylistName, createPlaylist }: CreatePlaylistSheetProps) {
...
function UploadSheet({ closeUpload, uploadFileInputRef, handleFileUpload, uploadPreviews, confirmUpload }: UploadSheetProps) {
...
function SongAddSheet({ setSongAddSheetOpen, availableSongs, selectedTrackIds, toggleSong, addSongsToPlaylist }: SongAddSheetProps) {
  const [songQuery, setSongQuery] = useState('')
  const filteredSongs = availableSongs.filter(item => item.title.toLowerCase().includes(songQuery.toLowerCase()) || item.artist.toLowerCase().includes(songQuery.toLowerCase()))
  return <div className="create-overlay" onClick={() => setSongAddSheetOpen(false)}><section className="song-add-sheet" onClick={event => event.stopPropagation()} aria-label="플레이리스트에 곡 추가"><header><h2>곡 추가</h2><button onClick={() => setSongAddSheetOpen(false)} aria-label="닫기"><img src={closeIcon} alt="" /></button></header><label className="sheet-search"><input value={songQuery} onChange={event => setSongQuery(event.target.value)} placeholder="검색" aria-label="추가할 곡 검색" /></label><div className="song-options">{filteredSongs.map(item => { const selected = selectedTrackIds.includes(item.id); return <button key={item.id} className="song-option" onClick={() => toggleSong(item.id)}><span className="artwork" style={item.coverUrl ? { backgroundImage: `url(${item.coverUrl})`, backgroundSize: 'cover' } : undefined} /><span className="track-copy"><strong>{item.title}</strong><span>{item.artist}</span></span><span className={selected ? 'circle-choice checked' : 'circle-choice'}>{selected ? '✓' : <img src={checkboxCircleIcon} alt="" />}</span></button> })}</div>{filteredSongs.length === 0 && <p className="no-results">추가할 수 있는 곡이 없어요.</p>}<button className="create-button" onClick={addSongsToPlaylist}>{selectedTrackIds.length}곡 추가</button></section></div>
}

type DeleteConfirmDialogProps = Pick<LibrarySheetsProps, 'playlistPendingDeleteId' | 'setPlaylistPendingDeleteId' | 'deletePlaylist'>

function DeleteConfirmDialog({ playlistPendingDeleteId, setPlaylistPendingDeleteId, deletePlaylist }: DeleteConfirmDialogProps) {
...
function UploadResultDialog({ uploadResult, closeUploadResult }: UploadResultDialogProps) {
